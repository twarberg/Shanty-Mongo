...
//		$value = $this->getFilters(self::DYNAMIC_INDEX)->filter($document);
	...
	public function export($applyOperations = false)
	{
		...
		$exportData = parent::export($applyOperations);
	...
	public function removeDocument(Shanty_Mongo_Document $document)
	{
		$index = array_serch($document, $this);
		if($index !== false) {
			return $this->setProperty($index, null);
		}
		return false;
	}
